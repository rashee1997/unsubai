...
export const ComposeModal: React.FC<ComposeModalProps> = ({
  ...
  const handleInsertSignature = () => {
    setBody((prev) => `${prev.trim()}\n\nBest regards,\nSent via Unsub.AI Humanized Mail`);
  };
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-xs">
      <div className="relative w-full max-w-2xl bg-white dark:bg-[#0c0c0e] rounded-2xl shadow-2xl border border-slate-200 dark:border-white/10 flex flex-col max-h-[90vh] overflow-hidden animate-in fade-in zoom-in-95 duration-200">
        ...
        <div className="p-6 flex-1 flex flex-col gap-4 overflow-y-auto">
          ...
          <div className="flex-1 flex flex-col min-h-[240px] border border-slate-200/80 dark:border-white/10 rounded-xl overflow-hidden bg-slate-50/50 dark:bg-zinc-900/50">
            {/* Formatting Toolbar */}
            <div className="px-3 py-2 bg-slate-100/70 dark:bg-zinc-800/60 border-b border-slate-200/80 dark:border-white/5 flex flex-wrap items-center justify-between gap-1 text-slate-600 dark:text-zinc-400">
              <div className="flex flex-wrap items-center gap-1">
                <button
                  type="button"
                  onClick={() => insertFormatting('**', '**')}
                  title="Bold (**text**)"
                  className="p-1.5 rounded-md hover:bg-slate-200/80 dark:hover:bg-zinc-700 hover:text-slate-900 dark:hover:text-white transition-colors"
                >
                ...
                <button
                  ...
                  className="px-2 py-1 rounded-md text-[11px] font-medium bg-white dark:bg-zinc-800 border border-slate-200 dark:border-zinc-700 hover:bg-slate-50 text-slate-700 dark:text-zinc-300 flex items-center space-x-1"
                >
              ...
              <div className="flex items-center space-x-1 bg-white dark:bg-zinc-800 p-0.5 rounded-lg border border-slate-200/80 dark:border-zinc-700">
                ...
                >
                  <PenSquare className="w-3 h-3 inline mr-1" />
                  Write
                </button>
                <button
                  type="button"
            ...
            {composerMode === 'write' ? (
              <textarea
                ...
                placeholder="Write your email here... (Supports rich text formatting, Markdown shortcuts, and AI Humanizer)"
                className="flex-1 w-full p-3.5 bg-transparent text-sm text-slate-900 dark:text-white placeholder-slate-400 focus:outline-hidden resize-none font-sans leading-relaxed min-h-[180px]"
              />
            ) : (
              <div className="flex-1 w-full p-4 bg-white dark:bg-zinc-950 text-sm text-slate-800 dark:text-zinc-200 overflow-y-auto min-h-[180px]">
                <div className="text-xs text-slate-400 dark:text-zinc-500 mb-2 font-mono uppercase tracking-wider">
                  Live Recipient HTML Render
                </div>
                <div
                  ...
                  dangerouslySetInnerHTML={{
                    __html: body
                      ? body
                          ...
                          .replace(/\*(.*?)\*/g, '<em>$1</em>')
                          .replace(/\n/g, '<br/>')
                      : '<em style="color: #94a3b8">No body text entered yet.</em>',
                  }}
                />
              </div>
            )}
